...
export async function getPeerBenchmark(councilName: string): Promise<PeerBenchmark | null> {
  try {
    ...
    const rows = await querySnowflake(`
      WITH own AS (
        SELECT COUNT_IF(OUTCOME IN ('GRANTED_FULL','GRANTED_PARTIAL','GRANTED_PARTIAL_S21','S21_REUSE'))
...
const RESPONSE_TYPES = new Set<ResponseType>(["DISCLOSURE", "PARTIAL", "REFUSAL", "S21_REUSE", "MIXED_S21"])
...
const RESPONSE_TYPE_TO_OUTCOME: Record<string, string> = {
  ...
  REFUSAL: "REFUSED",
  S21_REUSE: "S21_REUSE",
  MIXED_S21: "GRANTED_PARTIAL_S21",
}

/**
 * The dispositions engaged by a case. A response outcome is COMPOSED from these
 * rather than chosen by the first rule that matches — so "part already published,
 * part withheld, part new" is representable instead of being collapsed into a
 * single scalar label.
 */
export interface CaseDispositions {
  /** Some of the requested information is already published (s.21). */
  published: boolean
  publishedRef: string
  publishedPct: number | null
  /** Exemptions assessed as "apply" — information withheld. */
  withheldSections: string[]
  /** Exemptions assessed as "disclose" — information released. */
  releasing: boolean
}

/** Compose the dispositions into a single response type + human reason. */
export function composeResponseType(d: CaseDispositions): { type: ResponseType; reason: string } {
  const secs = d.withheldSections.join(", ")
  const pct = d.publishedPct == null ? "" : ` (${Math.round(d.publishedPct)}% match)`
  const pubPart = d.publishedRef ? `already published in ${d.publishedRef}${pct}` : `already published${pct}`

  // s.21 no longer short-circuits: if the case ALSO engages exemptions or has
  // information to release, the outcome is mixed.
  if (d.published && (d.withheldSections.length > 0 || d.releasing)) {
    const parts = [`part is ${pubPart}`]
    if (d.releasing) parts.push("part is released")
    if (d.withheldSections.length > 0) parts.push(secs ? `part is withheld under ${secs}` : "part is withheld under an exemption")
    return { type: "MIXED_S21", reason: `Mixed outcome: ${parts.join(", ")}.` }
  }
  if (d.published) {
    return { type: "S21_REUSE", reason: `Already published; ${pubPart}.` }
  }
  if (d.withheldSections.length > 0) {
    if (d.releasing) return { type: "PARTIAL", reason: secs ? `${secs} applies to part of the information.` : "Some information withheld under exemptions." }
...
export async function suggestedResponseType(
  ...
  const composed = composeResponseType({
    published: Boolean(triage?.s21MatchRef) || outcome === "S21_REUSE" || outcome === "GRANTED_PARTIAL_S21",
  ...
  // Only fall back to the recorded outcome when nothing was assessed either way.
  if (composed.type === "DISCLOSURE") {
...
export async function generateGroundedLetter(
  ...
  const typeGuidance: Record<ResponseType, string> = {
    ...
    MIXED_S21: "This is a MIXED response: part of the request is already published and part is not. Deal with the request in parts. For the part already published, apply s.21 and point the requester to exactly where to find it (named below) rather than re-supplying it. For the remaining part, give the substantive answer with the specific figures from the sources, and where an exemption applies to any of it, state the exemption and the public-interest balance. Make clear which part of the request each paragraph is answering. ",
  }
  const prompt =
  ...
  if (letter && caseId) {
    ...
    const s17ExemptionStated =
      type === "REFUSAL" || type === "PARTIAL" || type === "S21_REUSE" || type === "MIXED_S21"
...
export interface CaseTriage {
  ...
  /**
   * The unmodified CORTEX.SENTIMENT value. `sentimentScore` may have been pulled
   * into a tone band for DISPLAY; this is what gets persisted, so a cosmetic demo
   * choice never reaches SENTIMENT_SCORE (which feeds PRIORITY_SCORE and therefore
   * the officer work queue).
   */
  sentimentScoreRaw?: number | null
  sentimentRationale: string
  departments: string[]
  estimatedHours: number | null
  isVexatious: boolean
  ...
  s21MatchRef: string
  /** AI_SIMILARITY score (0-100) of the s.21 match; null when nothing cleared the threshold. */
  s21SimilarityPct: number | null
...
export async function getCaseTriage(caseId: string): Promise<CaseTriage | null> {
  const rows = await querySnowflake(`
    SELECT
      ...
      t.S21_SIMILARITY_PCT                  AS S21_PCT,
  ...
  return {
    ...
    s21SimilarityPct: r.S21_PCT == null ? null : Number(r.S21_PCT),
    scope: (() => {
      try {
...
//
// The bounds MUST sit wholly inside the display thresholds in format.ts
// (Negative < -0.3, Positive > 0.3), otherwise clamping a tone can render the
// opposite label to the one chosen — e.g. a 0.15 floor for "Polite" displayed as
// "Neutral". Bands are also kept non-overlapping.
const TONE_SENTIMENT_BAND: Record<string, { min: number; max: number; anchor: number }> = {
  Hostile: { min: -1.0, max: -0.6, anchor: -0.8 },
  Frustrated: { min: -0.55, max: -0.35, anchor: -0.45 },
  Neutral: { min: -0.15, max: 0.15, anchor: 0.0 },
  Polite: { min: 0.35, max: 0.55, anchor: 0.45 },
  Appreciative: { min: 0.6, max: 0.9, anchor: 0.75 },
}
...
export async function createIntakeCase(input: {
  ...
  // Persist the RAW sentiment, never the tone-clamped display value, and keep NULL
  // as NULL — "could not assess tone" must not be recorded as "neutral", and this
  // column feeds PRIORITY_SCORE.
  const rawSent = triage.sentimentScoreRaw !== undefined ? triage.sentimentScoreRaw : triage.sentimentScore
  const sentSql = rawSent == null ? "NULL" : String(rawSent)
  ...
  await querySnowflake(`
    ...
    SELECT '${escLit(reference)}','EMAIL','${escLit(senderName)}','${escLit(requestText)}',CURRENT_DATE(),
           '${regime}','CLASSIFY','OPEN','${escLit(dept)}',${deadlineSql},'RUNNING',${sentSql},
 ...
 */
/**
 * The most recently created email-intake case, used to RECOVER the intake demo
 * when the mailbox poll succeeds server-side but the HTTP response is lost.
 *
 * SP_POLL_OUTLOOK_INBOX can run for 90s+ (Graph round-trip plus five Cortex
 * functions per message) and marks mail as read as it goes, so a dropped
 * connection leaves a real, triaged case with no way for the client to learn its
 * reference. This lets the UI pick that case up and continue the pipeline instead
 * of dead-ending.
 */
export async function getLatestIntakeCase(
  withinMinutes = 20,
): Promise<{ reference: string; subject: string } | null> {
  const mins = Number.isFinite(withinMinutes) ? Math.max(1, Math.min(240, Math.floor(withinMinutes))) : 20
  const rows = await querySnowflake(`
    SELECT REFERENCE, SUBJECT
    FROM ${SCHEMA}.FOI_CASE
    WHERE SOURCE = 'EMAIL'
      AND CREATED_AT >= DATEADD(minute, -${mins}, CURRENT_TIMESTAMP())
    ORDER BY CREATED_AT DESC
    LIMIT 1
  `)
  if (!rows.length) return null
  return { reference: String(rows[0].REFERENCE ?? ""), subject: String(rows[0].SUBJECT ?? "") }
}
...
export async function getWdtkBenchmark(): Promise<WdtkBenchmarkRow[]> {
  ...
  const homeStats = await querySnowflake(`
    SELECT SUM(IFF(OUTCOME IN ('GRANTED_FULL','GRANTED_PARTIAL','GRANTED_PARTIAL_S21'),1,0))/NULLIF(COUNT(*),0) AS SUCCESS_RATE,
...
export async function getPublishableCases(): Promise<{ reference: string; subject: string }[]> {
  const rows = await querySnowflake(`
    ...
    WHERE STATUS='CLOSED' AND COALESCE(IS_PUBLISHED,FALSE)=FALSE
      AND OUTCOME IN ('GRANTED_FULL','GRANTED_PARTIAL','GRANTED_PARTIAL_S21')
