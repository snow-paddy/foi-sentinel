    ...
    with st.container(key="kpi_open"):
        ...
        with oc1:
            if st.button(f"{at_risk} at risk", key="chip_atrisk", use_container_width=True,
                         help="At risk — 5 working days or fewer to the statutory deadline"):
                _drill("At risk only")
        with oc2:
            if st.button(f"{overdue} overdue", key="chip_overdue", use_container_width=True,
                         help="Past the statutory deadline"):
                _drill("Overdue only")
        st.markdown("<div class='kpi-target'>◎ ‘At risk’ = 5 working days or fewer left</div>",
                    unsafe_allow_html=True)
with cc2:
    with st.container(key="kpi_closed"):
        ...
        with zc1:
            if st.button(f"{pct}% in time", key="chip_intime", use_container_width=True,
                         help=f"{pct}% answered within the statutory deadline, against a {target}% target"):
                _drill("Closed")
        with zc2:
            if st.button(f"{open_c + closed_c} total", key="chip_total", use_container_width=True,
                         help="All requests this period — open and closed"):
                _drill("All")
        st.markdown(f"<div class='kpi-target'>◎ Against the {target}% regulator target</div>",
                    unsafe_allow_html=True)
with cc3:
    with st.container(key="kpi_type"):
        st.markdown(
            "<div class='kpi-eyebrow'>Live</div>"
            f"<div class='stat-num-sm'>{int(k['FOI_O'])} · {int(k['EIR_O'])} · {int(k['SAR_O'])}</div>"
            f"<div class='stat-lbl'>Open by type — Information · Environmental · Subject Access</div>"
            f"<div class='kpi-note'>Different laws, different deadlines — see targets below.</div>",
            unsafe_allow_html=True)

# ---- Statutory deadlines callout ----
_shared.sla_callout()
...
with g:
    st.markdown("**Answered within the statutory deadline**")
    ...
    st.plotly_chart(style(gauge, 240, legend=False), use_container_width=True, config={"displayModeBar": False})
    (st.success if pct >= target else st.warning)(f"{pct}% answered in 20 working days (target {target}%)")

with p:
    st.markdown("**Where requests are in the process**")
    st.caption("Open requests by lifecycle stage — red shows those at risk of breaching the deadline.")
    pipe = session.sql(f"""
    ...
    fig = go.Figure()
    fig.add_bar(y=pipe["STAGE_NAME"], x=pipe["ON_TRACK"], name="On track", orientation="h", marker_color=BLUE)
    fig.add_bar(y=pipe["STAGE_NAME"], x=pipe["AT_RISK"], name="At risk", orientation="h", marker_color=RED)
    fig.update_layout(barmode="stack")
    fig.update_yaxes(autorange="reversed", title=None)
    fig.update_xaxes(title="open requests", dtick=1, gridcolor="#e8edf1")
    st.plotly_chart(style(fig, 360), use_container_width=True, config={"displayModeBar": False})
...
with ic1:
    ...
    extra = {"please", "provide", "information", "request", "council", "freedom", "act", "would", "like",
             "year", "years", "data", "details", "number", "last", "three", "bristol", "under", "foi", "copy", "also",
