...
export function OutlookTest({ mailbox }: { mailbox: string }) {
  ...
  async function runPipeline() {
    ...
    try {
      ...
      if (!syncRes.ok || !sync.ok) { setError(sync.error ?? "Mailbox sync failed"); setRunning(false); return }
      const cases: NewCase[] = sync.newCases ?? []
      if (!cases.length) {
      ...
      if (!pipeRes.ok || !pipe.ok) { setError(pipe.error ?? "Pipeline failed"); setRunning(false); peek(); return }
      setPipeline(pipe as Pipeline)

      // Notebook-style progressive reveal of stages 2..6.
      for (let n = 2; n <= 6; n++) { await sleep(750); setRevealed(n) }
      peek()
    } catch {
      setError("Could not reach the pipeline endpoint")
    } finally {
      setRunning(false)
    }
  }
