...
import {
  getCaseDetail, getCaseTimeline, getCaseExemptions, getLifecycleStages, getCaseTriage, getPrecedentMatch, getSarDocs, getFoiRedactions, getResponses, getAiAuditTrail, getCaseAiCost,
...
async function CaseView({ params }: { params: Params }) {
  ...
  // Pre-suggest the response type from triage + exemption assessments.
  ...
  let suggestedReason = "No exemptions engaged; release in full."
  if (triage?.s21MatchRef) {
    suggestedType = "S21_REUSE"
    suggestedReason = `Already published; matches ${triage.s21MatchRef}.`
  } else if (applied.length > 0) {
    const secs = [...new Set(applied.map((e) => e.sectionRef).filter(Boolean))].join(", ")
    if (disclosed.length > 0) {
      suggestedType = "PARTIAL"
      suggestedReason = secs ? `${secs} applies to part of the information.` : "Some information withheld under exemptions."
    } else {
      suggestedType = "REFUSAL"
      suggestedReason = secs ? `${secs} applies to the requested information.` : "Information withheld under exemptions."
    }
  }
  ...
  const VALID_TYPES: ResponseType[] = ["DISCLOSURE", "PARTIAL", "REFUSAL", "S21_REUSE"]
